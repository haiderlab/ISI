...
quitKey = KbName('Esc');
rewardToggleKey = KbName('f5');
leftToggleKey = KbName('f9');
rightToggleKey = KbName('f10');
rewardPulseKey = KbName('f6');
leftPulseKey = KbName('f11');
rightPulseKey = KbName('f12');
rewardCalibrationKey = KbName('f7');
leftCalibrationKey   = KbName('f3');
rightCalibrationKey  = KbName('f4');
timelineToggleKey = KbName('f8');
laserPulseKey = KbName('f2');
useTimeline = false;
rewardId = 1;
...
fprintf('\n<Esc> quit, <f5> toggle reward, <f8> toggle timeline\n');
fprintf('<%s> reward pulse, <%s> perform reward calibration <%s> Left Reward Calibration <%s> Right Reward Calibration <%s> Laser Pulse... <%s> Left Toggle <%s> Right Toggle <%s> Left Pulse <%s> Right Pulse\n',...
  KbName(rewardPulseKey), KbName(rewardCalibrationKey), KbName(leftCalibrationKey),...
  KbName(rightCalibrationKey),KbName(leftToggleKey),KbName(rightToggleKey),KbName(leftPulseKey),KbName(rightPulseKey),KbName(laserPulseKey));
...
while running
  ...
  if firstPress(leftToggleKey) > 0
      log('Toggling Left Reward valve');
      curr = rig.LeftdaqController.Value(rewardId);
      sig = rig.LeftdaqController.SignalGenerators(rewardId);
      if curr == sig.OpenValue
          rig.LeftdaqController.Value(rewardId) = sig.ClosedValue;
      else
          rig.LeftdaqController.Value(rewardId) = sig.OpenValue;
      end
  end
  
   if firstPress(rightToggleKey) > 0
    log('Toggling Right Reward valve');
    curr = rig.RightdaqController.Value(rewardId);
    sig = rig.RightdaqController.SignalGenerators(rewardId);
    if curr == sig.OpenValue
      rig.RightdaqController.Value(rewardId) = sig.ClosedValue;
    else
      rig.RightdaqController.Value(rewardId) = sig.OpenValue;
    end
  end
  ...
  if firstPress(leftPulseKey) > 0
    log('Delivering default Left reward');
    def = [rig.LeftdaqController.SignalGenerators(rewardId).DefaultCommand];
    %     def = rewardSig.DefaultCommand;
    rig.LeftdaqController.command(def);
  end
  
  if firstPress(rightPulseKey) > 0
    log('Delivering default Right reward');
    def = [rig.RightdaqController.SignalGenerators(rewardId).DefaultCommand];
    %     def = rewardSig.DefaultCommand;
    rig.RightdaqController.command(def);
  end
  ...
  if firstPress(leftCalibrationKey) >0
       log('Performing a Left reward delivery calibration');
    calibrateLeftWaterDelivery();
  end
  
   if firstPress(rightCalibrationKey) >0
       log('Performing a Right reward delivery calibration');
    calibrateRightWaterDelivery();
  end
  if(firstPress(laserPulseKey) >0)
 rig.laser.deliverBackground(0.5,1,0,1,.01);
        log('Pulsing Laser')
  end
...
function calibrateLeftWaterDelivery()
    LeftdaqController = rig.LeftdaqController;
    chan = LeftdaqController.ChannelNames(rewardId);
    %perform measured deliveries
    rig.scale.init();
    calibration = hw.calibrate(chan, LeftdaqController, rig.scale, 20e-3, 150e-3);
    rig.scale.cleanup();
    %different delivery durations appear in each column, repeats in each row
    %from the data, make a measuredDelivery structure
    ul = [calibration.volumeMicroLitres];
    ...
    save(rigHwFile, 'LeftdaqController', '-append');
    %     disp('TODO: implement saving');
    %save the updated rewardCalibrations struct
    %     save(, 'rewardCalibrations', '-append');
    %apply the calibration to rewardcontroller
...
function calibrateRightWaterDelivery()
    RightdaqController = rig.RightdaqController;
    chan = RightdaqController.ChannelNames(rewardId);
    %perform measured deliveries
    rig.scale.init();
    calibration = hw.calibrate(chan, RightdaqController, rig.scale, 20e-3, 150e-3);
    rig.scale.cleanup();
    %different delivery durations appear in each column, repeats in each row
    %from the data, make a measuredDelivery structure
    ul = [calibration.volumeMicroLitres];
    ...
    save(rigHwFile, 'RightdaqController', '-append');
    %     disp('TODO: implement saving');
    %save the updated rewardCalibrations struct
    %     save(, 'rewardCalibrations', '-append');
    %apply the calibration to rewardcontroller

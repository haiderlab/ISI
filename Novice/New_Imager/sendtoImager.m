...
global imagerhandles h FPS GUIhandles analyzer matroxObject
global dataTT dataTTReal analogIN cameraInterface ROIcrop
try
    hFigs = findall(groot, 'Type', 'figure');
    if strcmp(hFigs.Name,'Imager')
        close Imager
    end
catch
    fprintf('Imager window already closed \n')
end
...
switch(cmd(1))
    ...
    case 'S'  %% start sampling...
        ...
        if cameraInterface.useExecutable
            fprintf('Getting Frames using new Cam Interface \n')
            imagerhandles.m = 0; %disconnect matroxcam
            matroxObject = 0; %fully disconnect matroxcam
            [ims, timevecReal] = grabFrames(j, FPS, cameraInterface.frameDimensions, cameraInterface.exePath, cameraInterface.tempStoragePath, ROIcrop);
            fprintf('Done getting Frames using new Cam Interface \n')       
        else
            fprintf('Getting Frames using old Cam Interface \n')
            for i = 1:j %Capture images from feed
                % Recording frame time stamps
                c2 = clock;
                timevecReal = [timevecReal (c2(4)*3600 + c2(5)*60 + c2(6)) *1000];
                % Record & save snaps
                frame = snapshot(matroxObject);
                I = imcrop(frame,ROIcrop);
                ims = [ims {I}];
                pause((1/FPS)- 0.035)
            end
            fprintf('Done getting Frames using old Cam Interface \n')
        end
        c2 = clock;
        timeSt = [timeSt (c2(4)*3600 + c2(5)*60 + c2(6)) *1000];
        %%
        ...
        try
            ...
            fprintf('Feedback could not be performed \n')
            fprintf('Check parameters and Run again \n')
        end
